...
- 响应：`UserResponse`、`List<BookResponse>` 等由 Jackson 序列化；
...
**不用 DTO 可以吗？** 作业 5 起 **`Book` 亦通过 `BookResponse` 对外**；`User` 必须用 DTO（含 password）。购物车/订单沿用专用 DTO。
...
**DTO 与 Entity 的分离**：概念定义见 [0.10 DTO](#010-dto-datatransfer-object数据传输对象)。本项目里，`User` 含 `password` 必须用 `UserResponse` 对外；`Book` 通过 `BookResponse` 对外（作业 5）；购物车/订单用 DTO 裁剪字段或将 `orderNo` 映射为 JSON 的 `id`。
...
`BookController` 返回 `BookResponse` DTO：

```37:40:springboot-ebook/src/main/java/com/ebook/backend/controller/BookController.java
    @GetMapping("/books")
...
- 集合：`List<BookResponse>` → JSON array。
...
|------|------|-----------------|--------------|
| GET | `/api/v1/books` | `BookController.getAllBooks` | `BookService.listAll` |
| GET | `/api/v1/book/{id}` | `BookController.getBookById` | `BookService.getById` |
