...
> **文档类型**：技术设计与实现说明（非答辩提纲）  
> **覆盖范围**：`springboot-ebook` 模块；前后端协作仅说明后端契约与对接点。  
> **阅读方式**：**建议先读 [第零部分](#第零部分-基础概念导读是什么--为什么--本项目中的体现)**（各技术「是什么」），再读第一～十三部分（在本项目中的设计与实现）。代码路径均相对于仓库根目录 `EBook/`。

---

## 目录

- [第零部分 基础概念导读（是什么 · 为什么 · 本项目中的体现）](#第零部分-基础概念导读是什么--为什么--本项目中的体现)
...
- [第六部分 数据持久化：JDBC、连接池、SQL 脚本与 JPA](#第六部分-数据持久化jdbc连接池sql-脚本与-jpa)
- [第七部分 Spring Data JPA 与 Repository 机制](#第七部分-spring-data-jpa-与-repository-机制)
- [第八部分 领域模型：Entity 与数据库映射](#第八部分-领域模型entity-与数据库映射)
- [第九部分 DTO、校验与对外 API 契约](#第九部分-dto校验与对外-api-契约)
...
- 用 **HTTP 方法表示动作**（GET 查询、POST 创建/提交动作、PATCH 部分更新、DELETE 删除）；
- 用 **JSON** 作为资源的常见表示形式。

### 为什么需要

统一、可预测的接口让前端、测试工具、文档都能按同一套规则工作；HTTP 是互联网最通用的应用层协议。
...
Controller 类上的 `@GetMapping`、`@PostMapping` 等即把 Java 方法**绑定**到 REST 风格的 URL 与方法上（见第五部分）。
...
**JSON（JavaScript Object Notation）** 是一种**文本格式**，用来表示结构化数据，例如：
...
特点：键值对、数组、嵌套对象；人类可读；与编程语言无关（Java、JavaScript、Python 都能解析）。

### 为什么需要

HTTP body 需要一种双方都懂的格式。JSON 比 XML 简洁，成为 Web API 的事实标准。前端 `fetch` 与后端 Spring 都原生支持 JSON。

### 在本项目中的体现

- 请求：登录时 body 为 `{"username":"...","password":"..."}`，由 Jackson 反序列化为 `UserLoginRequest`；
- 响应：Controller 返回的 `UserResponse`、`List<Book>` 等由 Jackson **自动序列化**为 JSON 字符串发给浏览器。
...
- 库名：`ebook_backend`；
- 四张表：`users`、`books`、`cart_items`、`orders`（定义见 `schema.sql`）；
...
**连接池**预先创建若干条到数据库的 `Connection`，放在池中复用。请求来时**借**一条，用完**还**回池中，而不是每次 `new Connection()`。

**HikariCP** 是 Java 生态中性能较好的一种连接池实现；Spring Boot 2+ 默认采用它。

### 为什么需要

建立数据库连接涉及 TCP 握手、认证，成本高。高并发 Web 服务若每请求新建连接，数据库会很快成为瓶颈。

### 在本项目中的体现

启动日志中的 `HikariPool-1 - Starting...` 即池初始化。连接失败（库不存在、密码错误）时，池在启动阶段 `checkFailFast` 失败，应用无法就绪。
...
**Spring** 是 Java 生态的**应用开发框架**，核心能力包括：

- **IoC 容器**：管理对象的创建与依赖关系（见 0.14）；
- **AOP、事务、MVC** 等模块（本项目主要用 MVC 与事务基础设施）。

**Spring Boot** 是在 Spring 之上的**快速启动层**：内嵌 Web 服务器、自动配置数据源/JPA、约定 `application.properties`，减少 XML 与样板配置。

### 为什么需要

从零拼装 Spring + Tomcat + Hibernate 配置量大。Spring Boot 让「加一个 starter 依赖就能用 JPA」成为现实，适合课程与中小型服务。

### 在本项目中的体现

- 启动类：`@SpringBootApplication` 的 `EbookBackendApplication`；
- 依赖：`spring-boot-starter-web`、`spring-boot-starter-data-jpa` 等（`pom.xml`）；
- 配置：`application.properties` 驱动自动装配。
...
浏览器的**同源策略**规定：页面来自 `http://localhost:5173` 时，默认不能让 JavaScript 随意读取 `http://localhost:8080` 的响应（**不同端口即不同源**）。

**CORS** 是 W3C 机制：服务器在响应头中声明「允许哪些源、哪些方法访问」，浏览器才放行。

### 为什么需要

前后端分离必然跨源。没有 CORS 配置时，`fetch` 会在浏览器端失败，即使后端逻辑正确。

### 在本项目中的体现

`config/CorsConfig.java` 对 `/api/**` 允许源 `http://localhost:5173` 及常用 HTTP 方法。
...
**Maven** 是 Java 的**构建与依赖管理**工具：`pom.xml` 声明项目依赖哪些 jar，Maven 从中央仓库下载并参与编译、打包。

**Spring Boot Starter** 是一组**打包好的依赖集合**（如 `spring-boot-starter-web`），引入一个 starter 即引入一整套协调好的版本，并触发相关自动配置。

### 为什么需要

手动管理几十个 jar 的版本极易冲突。Starter + Parent POM 解决「能跑起来」的最低成本。

### 在本项目中的体现

`pom.xml` 中四个 starter（web、data-jpa、mysql、validation）；`mvn package` 生成可执行 jar。

---

## 0.21 事务（Transaction）概念
...
## 0.22 `Optional` 与 `ResponseEntity`
...
`orderRepository.saveAll(orders)` 在 Hibernate 中可能生成多条 INSERT；是否批处理取决于 `hibernate.jdbc.batch_size` 等（本项目未调优）。`createOrders` 循环内为每个购物车项创建一条 `OrderEntity`，订单与购物车项 **1:1** 对应（一单书一条订单记录）。
...
| `orders` | `OrderEntity` | 类名避让用户模块 `User` 等；表名仍为 `orders` |
...
## 8.5 `OrderEntity` 的价格快照

```67:69:springboot-ebook/src/main/java/com/ebook/backend/service/OrderService.java
            order.setQty(item.getQty());
            order.setUnitPrice(book.getPrice());
            order.setStatus("pending");
```

`unit_price` 写入时刻从 `books.price` 读取，之后改书价不影响历史订单——这是订单系统的常见 **快照（snapshot）** 设计。

订单号生成：

```84:88:springboot-ebook/src/main/java/com/ebook/backend/service/OrderService.java
...
`OrderResponse.id` 映射 `orderNo`（业务主键），而非表自增 `id`：

```91:97:springboot-ebook/src/main/java/com/ebook/backend/service/OrderService.java
...
前端用 `order.id` 展示订单号、调用 `PATCH .../{orderNo}`，与数据库 `order_no` 列一致。
...
| Response DTO | `UserResponse`, `CartItemResponse`, `OrderResponse` | 服务端 → 客户端 |
...
-- 2. 对每个 item INSERT INTO orders (...)
-- 3. DELETE FROM cart_items WHERE id IN (...)
```
...
[CartService.checkout(1)]
    ...
    │     ├─ for each item: bookRepository.findById
    │     ├─ new OrderEntity, set orderNo, unitPrice, status=pending
    │     └─ orderRepository.saveAll
    └─ cartItemRepository.deleteAll(selectedItems)
    ↓
[List<OrderResponse>] → JSON
    ↓
